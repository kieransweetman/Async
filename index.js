...
// user generated search for city
submit.addEventListener('click', (e) => {
    e.preventDefault();
    let city = input.value;
    cityReq(city)
        .then(data => getCityInfo(data, city))
        .then(city => printUsercity(city[0], city[1])).catch(e => { console.log(e); return alert(`${city} was not typed correctly \ncity not found.`) });
})
...
function getCityInfo(data, city) {
    if (data[0].nom.toLowerCase() === city.toLowerCase()) {
        return [data[0].nom, data[0].code];
    }
...
/// html + information parser functions
...
async function printUsercity(place, code) {
    if (place && code) {
        let meteoObj = await requestMeteo(code);
        let weatherToday = await meteoObj.forecast[0].weather;

        let airQuality = await requestAirQuality(place);
        let airData = await airQuality.data;
